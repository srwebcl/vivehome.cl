@csrf
{{-- Fila 1: Título y Tipo de Operación --}}
<div class="row mb-3">
    <div class="col-md-8">
        <label for="title" class="form-label">Título de la Propiedad <span class="text-danger">*</span></label>
        <input type="text" id="title" name="title" class="form-control" value="{{ old('title', $property->title ?? '') }}" required>
    </div>
    <div class="col-md-4">
...
<div class="row mb-3">
    ...
    <div class="col-md-4">
        ...
        <input type="number" id="price" name="price" class="form-control" value="{{ old('price', $property->price ?? '') }}" required>
    </div>
    <div class="col-md-4">
...
<div class="row mb-3">
    <div class="col-md-4">
        ...
        <input type="text" id="commune" name="commune" class="form-control" value="{{ old('commune', $property->commune ?? '') }}">
    </div>
    <div class="col-md-4">
        <label for="city" class="form-label">Ciudad</label>
        <input type="text" id="city" name="city" class="form-control" value="{{ old('city', $property->city ?? '') }}">
    </div>
    <div class="col-md-4">
        <label for="region" class="form-label">Región</label>
        <input type="text" id="region" name="region" class="form-control" value="{{ old('region', $property->region ?? '') }}">
    </div>
</div>
...
<div class="row mb-3">
    <div class="col-md-6">
        <label for="total_area_m2" class="form-label">Superficie Total (m²)</label>
        <input type="number" step="0.01" id="total_area_m2" name="total_area_m2" class="form-control" value="{{ old('total_area_m2', $property->total_area_m2 ?? '') }}">
    </div>
    <div class="col-md-6">
        <label for="built_area_m2" class="form-label">Superficie Construida (m²)</label>
        <input type="number" step="0.01" id="built_area_m2" name="built_area_m2" class="form-control" value="{{ old('built_area_m2', $property->built_area_m2 ?? '') }}">
    </div>
</div>

{{-- Fila 5: Habitaciones, Baños, etc. --}}
<div class="row mb-3">
    <div class="col-md-3">
    ...
    <div class="col-md-3">
        <label for="parking_lots" class="form-label">Estacionamientos</label>
        <input type="number" id="parking_lots" name="parking_lots" class="form-control" value="{{ old('parking_lots', $property->parking_lots ?? '0') }}">
    </div>
    <div class="col-md-3">
        <label for="storage_units" class="form-label">Bodegas</label>
        <input type="number" id="storage_units" name="storage_units" class="form-control" value="{{ old('storage_units', $property->storage_units ?? '0') }}">
    </div>
</div>
...
<div class="mb-3">
    ...
    <textarea id="description" name="description" class="form-control" rows="5" required>{{ old('description', $property->description ?? '') }}</textarea>
</div>

<hr class="my-4">

{{-- INICIO: SECCIÓN DE MULTIMEDIA MEJORADA --}}
<h5 class="mb-3">Multimedia</h5>

{{-- Sub-sección para mostrar y eliminar imágenes existentes --}}
    ...
    <div class="row g-2 mb-3">
        @foreach($property->photos as $photo)
            <div class="col-6 col-sm-4 col-md-3">
                <div class="card">
                    <img src="{{ Storage::url($photo->file_path) }}" class="card-img-top" alt="Foto de la propiedad" style="height: 120px; object-fit: cover;">
...
{{-- Sub-sección para cargar nuevas imágenes --}}
<div class="mb-3">
...
<div class="mb-3">
    ...
    <input 
        ...
        value="{{ old('video_url', $property->videos->first()->video_url ?? '') }}"
    >
    @error('video_url')
...
<div class="row">
    @php
        $checkedFeatures = old('features', isset($property) ? $property->features->pluck('id')->toArray() : []);
    @endphp
...
{{-- INICIO: Nueva Sección de Campos Personalizados --}}
<h5 class="mb-3">Campos Personalizados</h5>
<div class="row">
    @php
        // Creamos un mapa de los valores existentes para fácil acceso (ID de campo => valor).
        $existingCustomValues = isset($property) ? $property->customFieldValues->pluck('value', 'custom_field_definition_id') : collect();
    @endphp
    ...
    @foreach($customFields as $field)
        <div class="col-md-6 mb-3">
            ...
            @php
                // Obtenemos el valor guardado o el valor de un formulario anterior que falló.
                $value = old('custom_fields.' . $field->id, $existingCustomValues->get($field->id) ?? '');
            @endphp
            ...
            @else {{-- Por defecto, usamos un input de texto para 'text', 'date', 'boolean', etc. --}}
                <input type="text" class="form-control" id="custom_field_{{ $field->id }}" name="custom_fields[{{ $field->id }}]" value="{{ $value }}">
            @endif
        </div>
    @endforeach
</div>
{{-- FIN: Nueva Sección de Campos Personalizados --}}
...
<div class="mt-4">
    ...
    <a href="{{ route('asesor.properties.index') }}" class="btn btn-secondary">Cancelar</a>
</div>